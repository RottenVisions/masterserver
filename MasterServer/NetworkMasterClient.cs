...
namespace RottenVisions.Networking.MasterServer
{
	...
	public class NetworkMasterClient : MonoBehaviour
	{
		...
		public void RegisterHost(string gameTypeName, string gameName, string comment, bool passwordProtected,
			int playerLimit, int port)
		{
			if (!isConnected)
			{
		...
		void OnGUI()
		{
			...
			{
				MasterServerIpAddress = GUI.TextField(new Rect(100, 20, 200, 20), MasterServerIpAddress);
				if (GUI.Button(new Rect(100, 20 + yoffset + 20, 200, 20), "MasterClient Connect"))
				{
					InitializeClient();
				}
			...
			if (hosts != null)
			{
				...
				foreach (var h in hosts)
				{
					if (GUI.Button(new Rect(120, y + yoffset, 240, 20),
						"Host:" + h.name + "addr:" + h.hostIp + ":" + h.hostPort))
					{
						if (NetworkManager.singleton != null)
						{
